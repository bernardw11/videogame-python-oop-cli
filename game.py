...
class Player:
    ...
    def attack(self, target):
        ...
        power = random.randint(self.strength - 2, self.strength + 3)
        ...
        if self.strength < target.defense:
            ...
            damage = power
        print(f"The attack does {damage} damage.")
    ...
    def decrease_health_by(self, damage):
        if damage >= self.health:
            print(f"{self.name} lost {self.health} health.")
            self.alive = False
            self.health = 0
        else:
    ...
    def rest(self):
        print(f"{self.name} has {self.health} out of {self.max_health}")
        print(f"{self.name} heals 10 health")
        self.health += 10
    ...
    def special_attack(self, target):
        print(f"{self.name} does a super duper powerful atack!!!!!!")
    ...
    def level_up(self):
        self.level += 1
        self.strength += random.randint(0, 4)
...
class Battle:
    ...
    def start(self):
        ...
        while self.player1.health > 0 and self.player2.health > 0:
            if player.can_move:
                choice = int(input(f"{player.name}, what do you want to do? \n1 - attack \n2 - special attack \n3 - rest\n"))
                if choice == 1:
                    player.attack(other)
                elif choice == 2:
                    player.special_attack(other)
                elif choice == 3:
                    player.rest()
            else:
